...
<div class="slider-box">
  ...
  <span class="time-range">
    <span id="<?= $arSubfilter['ITEM_PREFIX'] ?>filter-from"><?= $arSubfilter['MIN_VALUE_TEXT'] ?></span>
    &#151;
    <span id="<?= $arSubfilter['ITEM_PREFIX'] ?>filter-till"><?= $arSubfilter['MAX_VALUE_TEXT'] ?></span>
...
<li<?= ($arApt['~DISABLED'] ? ' class="disabled"' : '') ?>><input type="checkbox" checked="checked"<?= ($arApt['~DISABLED'] ? ' disabled="disabled"' : '') ?> id="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>" />
...
<?
  ...
  case 'CARRIER':
    ...
    foreach ($arFilter['ITEMS'] as $arCarrier) {
      ?><li<?= ($arCarrier['~DISABLED'] ? ' class="disabled"' : '') ?>><input type="checkbox" checked="checked"<?= ($arCarrier['~DISABLED'] ? ' disabled="disabled"' : '') ?> id="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>" />
      <label class="logo-small-<?= $arCarrier['IATACODE'] ?>" for="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>"><?= $arCarrier['TITLE'] ?> (<?= $arCarrier['CRTCODE'] ?>)</label></li>
<?
  ...
  case 'SERVICE_CLASS':
    ...
    foreach ($arFilter['ITEMS'] as $arServiceClass) {
      ?><li<?= ($arServiceClass['~DISABLED'] ? ' class="disabled"' : '') ?>><input type="checkbox" checked="checked"<?= ($arServiceClass['~DISABLED'] ? ' disabled="disabled"' : '') ?> id="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>" />
      <label for="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>"><?= $arServiceClass['SERVICE_CLASS'] ?></label></li>
<?
  ...
  case 'TRANSFERS':
    ...
    foreach ($arFilter['ITEMS'] as $arTransfers) {
      ?><li<?= ($arTransfers['~DISABLED'] ? ' class="disabled"' : '') ?>><input type="checkbox" checked="checked"<?= ($arTransfers['~DISABLED'] ? ' disabled="disabled"' : '') ?> id="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>" />
      <label for="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>"><?= GetMessage('TS_IBE_OFFER_FILTER_TRANSFERS_FILTER_' . $arTransfers['TRANSFERS']); ?></label></li>
<?
  ...
  case 'PENALTY_TYPE':
    ...
    <ul>
      ...
      <li<?= ($arServiceClass['~DISABLED'] ? ' class="disabled"' : '') ?> style="display: none">
        ...
        <label for="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>"><?= $arPenaltyType[ 'PENALTY_TYPE' ] ?></label>
      </li>
      ...
      <li<?= ($arServiceClass['~DISABLED'] ? ' class="disabled"' : '') ?>>
        ...
        " />
        <label for="<?= $arFilter['ITEM_PREFIX'] ?><?= $itemIndex ?><?= $arResult[ "~UID" ] ?>_contrast"><?= $arPenaltyType[ 'PENALTY_TYPE_CONTRAST' ] ?></label>
      </li>
      <? $itemIndex++; ?>
      <? endforeach; ?>
    </ul>
