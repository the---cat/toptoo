...
<div class="delivery">
  <? //trace($arResult['FORM']['FIELDS']['DELIVERYMETHODS']) ?>
  <form name="<?= $arResult['FORM']['NAME']; ?>" action="<?=$arResult['FORM']['ACTION']; ?>" id="<?=$arResult['FORM']['~ID']; ?>" method="<?=$arResult['FORM']['~METHOD']; ?>"<? if ('Y' == $arParams['~IBE_AJAX_MODE']) { ?> onsubmit="ibe_ajax.post(this);return false;"<? } ?>>
    ...
    <br /><h3 class="info_caption"><?= $arResult['DISPLAY_ERROR']?></h3>
    <? endif; ?>
    ...
    <? if (isset($arResult['FORM']['FIELDS'])): ?>
      ...
      <div id="select_location_block" class="location" style="display:none;">
        <? if (1 == count($arResult['FORM']['FIELDS']['select_location']['OPTION'])
        && 'display:none;' == $arResult['FORM']['FIELDS']['select_location']['STYLE']
        && '' != $arResult['FORM']['FIELDS']['select_location']['OPTION'][0]['CAPTION']
        ): ?>
        ...
        <? endif; ?>
        <? if (array_key_exists('select_location', $arResult['FORM']['FIELDS'])
        && count($arResult['FORM']['FIELDS']['select_location']['OPTION']) > 1): ?>
      ...
      <div id="DeliveryMethod" class="delivery_methods">
        ...
        <div class="<?= $field['CLASS']; ?><?= $bShowCustomSelf || $bShowCustomCour ? ' delivery_groups' : '' ?>" id="<?= $field['~ID']; ?>">
          ...
          <? if ( $bShowCustomSelf || $bShowCustomCour ): ?>
          <? if ( $bShowCustomSelf ) { ?>
          <? $f2 = $ar_delivery_by_groups[$k]['SELF'][0]; ?>
          <div class="<?= $f2['CLASS']; ?> clearfix"<?=$f2['~id'] ? ' id="' . $f2['~id'] . '" ' : '' ?> onclick="<?= $f2['ONCLICK']; ?>;ViewAddresFormB2B('<?=$BlockID?>');">
            ...
            <div class="description_left">
              <div><?= GetMessage('IBE_FRONTOFFICE_DELIVERY_SELF_S') ?></div>
            </div>
            <div class="description_right">
              <div id="adr_<?=$f2['FIELDS'][0]['VALUE']; ?>" class="address">
                <? $deliveryDesc = '';
                $deliveryDesc .= (1 == count($arResult['FORM']['FIELDS']['select_location']['OPTION']) && 'display:none;' == $arResult['FORM']['FIELDS']['select_location']['STYLE'] && '' != $arResult['FORM']['FIELDS']['select_location']['OPTION'][0]['CAPTION'] ) ? $arResult['FORM']['FIELDS']['select_location']['OPTION'][0]['CAPTION'] . ', ' : '';
                $deliveryDesc .= implode('</div><div>', $f2['FIELDS'][0]['DESCRIPTION']); ?>
                <div>
                  <?= $deliveryDesc; ?>
                </div>
              </div>
          ...
          <? 
              unset($field['FIELDS'][ $ar_delivery_by_groups[$k]['SELF'][0]['METHOD_KEY'] ]);
              unset( $ar_delivery_by_groups[$k]['SELF']);  
            } ?>
          <? if ( $bShowCustomCour ) { ?>
          ...
          <div class="<?= $f2['CLASS']; ?> method_other"<?=$f2['~id'] ? ' id="' . $f2['~id'] . '" ' : '' ?> onclick="<?= $f2['ONCLICK']; ?>;ViewAddresFormB2B('<?=$BlockID?>');">
            <? foreach ($f2['FIELDS'] as $f3): ?>
            <input<?= ($f3['CHECKED'] ? ' checked="checked"' : '') ?> type="<?= $f3['~TYPE']; ?>" class="<?= $f3['CLASS']; ?>" id="<?= $f3['~ID']; ?>" value="<?= $f3['VALUE']; ?>" name="<?= $f3['NAME']; ?>" />
            <label for="<?= $f3['~ID']; ?>" class="title" title="<?= $f3['TITLE']; ?>">
              <?= $f3['CAPTION']; ?>
            </label>
            <? endforeach; ?>
          </div>
