...
$group_names = array();
$bNoGorup = false;
foreach ( $form['FIELDS'] as $pay_method_num => $method ) {
  if ( isset($method['INPUT_VALUE']) ) {
    if ( !$method['GROUP_NAME'] ) {
      $bNoGorup = true;
      continue;
    }
    if ( $method['GROUP_NAME'] && !in_array($method['GROUP_NAME'], $group_names) ){
      $group_names[] = $method['GROUP_NAME'];
    }
  }
}
if ( $bNoGorup && count($group_names) ){ $group_names[] = 'NO_GROUP'; }
  
$method_by_groups = array();    
  ...
  if ( count($group_names) ) {
    if ( isset( $method['INPUT_VALUE'] ) ) {
      ...
      if ( FALSE !== $k = array_search($group_name, $group_names) ) {
        $method_by_groups[$k]['GROUP_NAME'] = strlen(GetMessage('IBE_FRONTOFFICE_PAY_METHOD_GROUP_' . $group_name)) ? GetMessage('IBE_FRONTOFFICE_PAY_METHOD_GROUP_' . $group_name) : $group_name;
        $method_by_groups[$k]['GROUP_CLASS'] = $method['~SELECTED'] && empty($method_by_groups[$k]['GROUP_CLASS']) ? ' opened no_close' : $method_by_groups[$k]['GROUP_CLASS'];
        $method_by_groups[$k]['PAYMETHODS'][] = array(
          'GROUP_ID' => $k,
...
ksort(&$method_by_groups);
$method_by_groups = count($method_by_groups) ? $method_by_groups : false;

//trace($form['FIELDS']);
//trace($method_by_groups);
      ...
      <label for="<?=$method['~ID'] ?>">
        ...
        <div style="display:none" id="paysystem_profile_<?=$method['~ID'] ?>">
          ...
          if ( (count($method['DESCRIPTION']) > 1 && count($method['DESCRIPTION']) > $nSubSys = count($method['SUBSYSTEMS'])) || count($method['DESCRIPTION']) == $nSubSys ){
            $pcsMsg = '<ul><li>' . implode('</li><li>', $method['DESCRIPTION']) . '</li></ul>';
          }
          ...
          elseif ( $method[ "PS_CASH_TYPE" ] !== 'COUR' && ( ($method[ "ACTION" ] == "platron" && $method[ "SUBSYSTEM_ID" ]== 'POSTPONED') || substr($method['PS_TYPE'], 0,11) !== 'ONLINE_SYNC' ) ) :
            $ps_class = 'CASH';
          endif; ?>
          <? if ( $ps_class || $method['DESCRIPTION'] || array_key_exists($method['~ID'].'_div', $form['FIELDS'])): ?>
            ...
            <div class="description_right">
              <? // Опции
              $id = $method['~ID'];
              if ( array_key_exists($id.'_div', $form['FIELDS']) ) { ?>
              <div id="pay_method_options-<?=$id ?>" class="pay_method_options">
                <? foreach($form['FIELDS'][$id.'_div']['FIELDS'] as $option): ?>
                <? if($option['~TYPE'] == "text"): ?>
                <div class="option">
          ...
          <td class="paymethods">
            ...
            <? if ( $method_by_groups ): ?>
              <div class="pay_methods_groups">
                <? foreach ( $method_by_groups as $group_id => $ar_group ): ?>
                <h2 class="group_name<?= $ar_group['GROUP_CLASS'] ?>" id="group_<?= $group_id ?>"><span class="name"><?= $ar_group['GROUP_NAME'] ?></span></h2>
                <div class="paymethods_group<?= $ar_group == end($method_by_groups) ? ' last' : '' ?><?= strlen($ar_group['GROUP_CLASS']) ? ' opened' : '' ?>" id="paymethods_group_<?= $group_id ?>"<?= strlen($ar_group['GROUP_CLASS']) ? '' : ' style="display:none;"' ?>>
...
$('.paymethods .item input:radio').click(function(){
  ...
  var groupNameId = el.closest('.paymethods_group').attr('id');
