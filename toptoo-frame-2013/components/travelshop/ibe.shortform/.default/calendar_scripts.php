...
// ѕеренесено из form_order.php дл€ использовани€ и на других экранах фронт-офиса
...
// ”становка календарей (при загрузке страницы, в т.ч., через AJAX)
...
<? $arMinDate = getdate(CIBEOffer_getTimeLimit() - 12 * 3600); ?>
  ...
  if (typeof tooltip == 'function') {
    /* «амена исходной функции Datepicker'a */
...
function selectForwardDate<?= $form ?>(dateText) {
  ...
  // если дата вылета становитс€ больше даты возврата, то добавл€ем к дате возврата разницу дней между между датой возврата по умолчанию и датой вылета по умолчанию
...
function selectBackDate<?= $form ?>(dateText) {
  // если дата вылета становитс€ больше даты возврата, устанавливаем дату вылета на день раньше
...
// ѕреобразование строки с датой в формате сайте в объект javascript-даты
