(function($,undefined){$.extend($.ui,{datepicker:{version:"1.8.13"}});
...
settings[name]=value
}if(inst){if(this._curInst==inst){}var date=this._getDateDatepicker(target,true);
...
var $listWrapper=$("#selectScrollWrapper");
var $customSelect=$("#selectScrollMask");
var $customSelectItems=$("#customDatePickerSelect").find("a");
var sliderPosition=0;
var startYear=new Date().getFullYear();
var keyCharDown="",keyCharUp="";
$("#customDatePickerSelectTrigger").bind("click",function(){$listWrapper.toggle();
$(this).focus();
return false
});
$customSelectItems.bind({mousedown:function(event){$(this).trigger("click");
return false
},focus:function(event){keyCharDown="";
keyCharup="";
$(this).addClass("listItemFocus");
var listItemTop=$(this).position().top;
var focusedMonth=parseInt($(this).attr("id").split("month")[1].split("year")[0],10);
var focusedYear=parseInt($(this).attr("id").split("year")[1],10);
var monthInList=focusedMonth+((focusedYear-startYear)*12);
if(monthInList<$customSelectItems.length-4){$customSelect.scrollTop($(this).position().top)
}lastFocused=$(this).attr("class");
return false
},keydown:function(event){if(event.which=="16"){keyCharDown=event.which
}},keyup:function(event){keyCharUp=event.which
},blur:function(event){$(this).removeClass("listItemFocus")
}});
var numMonths=this._getNumberOfMonths(inst);
var cols=numMonths[1];
var width=17;
if(cols>1){inst.dpDiv.addClass("ui-datepicker-multi-"+cols).css("width","680px")
}var numMonths=this._getNumberOfMonths(inst);
var cols=numMonths[1];
var width=34;
...
drawYear--
}if(maxDate){var maxDraw=this._daylightSavingAdjust(new Date(maxDate.getFullYear(),maxDate.getMonth(),maxDate.getDate()));
...
}calender+='<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix'+cornerClass+'">'+(/all|left/.test(cornerClass)&&row==0?(isRTL?next:prev):"")+(/all|right/.test(cornerClass)&&row==0?(isRTL?prev:next):"")+this._generateMonthYearHeaderNew(inst,drawMonth,drawYear,minDate,maxDate,row>0||col>0,monthNames,monthNamesShort)+'</div><table class="ui-datepicker-calendar"><thead><tr>';
...
return html
},_generateMonthYearHeaderNew:function(inst,drawMonth,drawYear,minDate,maxDate,secondary,monthNames,monthNamesShort){var currentDate=new Date();
var currentYear=currentDate.getFullYear();
var currentMonth=currentDate.getMonth();
var disabledMonths=(minDate.getFullYear()-currentYear)*12+(minDate.getMonth()-currentMonth);
var maxDate=this._getMinMaxDate(inst,"max");
var numberOfMonths=12;
var customListItems="";
if(maxDate>currentDate){numberOfMonths=(maxDate.getFullYear()-currentYear)*12+(maxDate.getMonth()-currentMonth)
}for(var i=0;
i<=numberOfMonths;
i++){var monthAdjustment=(currentMonth-drawMonth)+((currentYear-drawYear)*12),monthDisabledString=(i<disabledMonths)?'<li class="disabled">':"<li>";
if((currentMonth==drawMonth)&&(currentYear==drawYear)){customListItems+=monthDisabledString+'<a href="#" class="currentMonthYear" id="month'+currentMonth+"year"+currentYear+'" onclick="DP_jQuery_'+dpuuid+".datepicker._adjustDate('#"+inst.id+"', +"+monthAdjustment+", 'M'); return false\">"+monthNames[currentMonth]+" "+currentYear+"</a></li>"
}else{if(i==(numberOfMonths-1)){customListItems+=monthDisabledString+'<a href="#" class="lastMonthYear" id="month'+currentMonth+"year"+currentYear+'" onclick="DP_jQuery_'+dpuuid+".datepicker._adjustDate('#"+inst.id+"', +"+monthAdjustment+", 'M'); return false\">"+monthNames[currentMonth]+" "+currentYear+"</a></li>"
}else{customListItems+=monthDisabledString+'<a href="#" id="month'+currentMonth+"year"+currentYear+'" onclick="DP_jQuery_'+dpuuid+".datepicker._adjustDate('#"+inst.id+"', +"+monthAdjustment+", 'M'); return false\">"+monthNames[currentMonth]+" "+currentYear+"</a></li>"
}}if(currentMonth==11){currentMonth=0;
currentYear++
}else{currentMonth++
}}var staticTitleHtml='<span class="customDatePickerTitle"><span class="titleMonth">'+monthNames[drawMonth]+"</span> "+drawYear+"</span>";
var customListHtml='<div id="customDatePickerSelectWrapper"><a href="#" id="customDatePickerSelectTrigger"><span class="selectTriggerWrapper"><span class="triggerMonth">'+monthNames[drawMonth]+"</span> "+drawYear+'</span></a><div id="selectScrollWrapper"><div id="selectScrollMask"><ul class="customDatePickerSelect" id="customDatePickerSelect">'+customListItems+"</ul></div></div></div>";
if(!secondary){return customListHtml
}else{return staticTitleHtml
